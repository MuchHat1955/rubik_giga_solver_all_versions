...
// ----------------------------------------------------------
// TODO use the openrb_controller
// ----------------------------------------------------------
...
void setup() {
  Serial.begin(115200);
  openrb.begin(); // auto sets VERBOSEOFF

  Serial.println("GIGA connected to OpenRB (verbose off mode)");

  // Move Y and read back
  XYStatus xy;
  if (openrb.moveToYSync(80.0, &xy)) {
    Serial.printf("Y=%.2f  X=%.2f  a1Ticks=%d a2Ticks=%d\n",
                  xy.y_mm, xy.x_mm, xy.a1_ticks, xy.a2_ticks);
  }

  // Move single servo and read back
  ServoStatus s;
  if (openrb.moveServoSync(11, 2100, &s))
    Serial.printf("Servo %d pos=%d\n", s.id, s.pos);

  // Read all servos at any time
  ServoStatus servos[10];
  if (openrb.readServosSync(servos, 10)) {
    for (int i = 0; i < 6; i++)
      if (servos[i].valid)
        Serial.printf("ID=%d pos=%d\n", servos[i].id, servos[i].pos);
  }

  // Query XY without moving
  XYStatus now;
  if (openrb.readXYSync(now))
    Serial.printf("Current XY: X=%.2f  Y=%.2f\n", now.x_mm, now.y_mm);
}
*/
