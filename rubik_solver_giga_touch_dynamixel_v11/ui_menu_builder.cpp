...
void buildMenu(const char *menuName) {
  ...
  if (strcmp(menuName, "main") == 0) {
    String sketchVer = getSketchVersionWithDate();
  ...
  // ---------- FOOTER (create once, reuse later) ----------
  if (!footLbl) {
  ...
  for (JsonArray row : rows) {
    ...
    for (int ci = 0; ci < numCols && ci < columns; ci++) {
      ...
      // Each widget type section below remains identical to your previous version
      // (TEXT / ACTION / NUM / STATUS_BUTTON / STATUS_TEXT)
      // ----------------------------------------------------------
      //                   TEXT
      // ----------------------------------------------------------
