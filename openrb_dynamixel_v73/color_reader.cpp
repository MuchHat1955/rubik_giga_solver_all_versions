...
void CubeColorReader::fill_solved_cube_top2layers_() {
  ...
  char centers[6];
  for (int f = 0; f < 6; f++) {
    char c = colors_[center_idx[f]];
    if (c == '.' || c == 0) {
      centers[f] = default_centers[f];
    } else {
      centers[f] = c;
    }
  }

  // 3) Overwrite faces using effective centers
  for (int f = 0; f < 6; f++) {
    int base = f * 9;
    ...
    if (f == 0) {
      for (int i = 0; i < 9; i++) {
        colors_[base + i] = centers[f];
      }
      continue;
    }

    // D face: untouched (all '.')
    if (f == 3) {
      continue;
    }

    // Side faces: first 6 stickers filled, last 3 unknown
    for (int i = 0; i < 6; i++) {
      colors_[base + i] = centers[f];
    }
  }
