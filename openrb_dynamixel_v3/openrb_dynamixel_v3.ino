...
void smallSingleServoMove(uint8_t id,
                          int rel_ticks,
                          uint32_t max_time_ms = 600,
                          int overshoot_ticks = 8) {
  if (!dxl.ping(id)) return;
  if (rel_ticks == 0) return;

  dxl.writeControlTableItem(ControlTableItem::MOVING_THRESHOLD, id, 1);
  dxl.writeControlTableItem(ControlTableItem::PROFILE_VELOCITY, id, 1000); // 0 – 1023 // was 100
  dxl.writeControlTableItem(ControlTableItem::PROFILE_ACCELERATION, id, 30000); //0 – 32767 // was 30
  ...
  Serial.print(" goal=");
  Serial.print(goal);
  Serial.print(" overshoot goal=");
  Serial.println(overshoot_goal);
...
void loop() {
  ...
  if (line.startsWith("MOVEY")) {
    ...
    smallSingleServoMove(id, rel, 500, 8); //TODO
