...
int getServoTicksForAngle(const std::string& servo, float f_angle_deg) {
  ...
  long t = 0;
  if (inv) t = z + (long)((f_angle_deg * (float)XL430_360DEG_TICKS) / 360.0);
  else t = z - (long)((f_angle_deg * (float)XL430_360DEG_TICKS) / 360.0);
  if (t < 0) t = 0;
  if (t > 4095) t = 4095;
  if (t > max) t = max;
  if (t < min) t = min;
...
void rotateToAngle(const std::string& name, int angle_deg) {
  if (!hasDynamixel) return;
