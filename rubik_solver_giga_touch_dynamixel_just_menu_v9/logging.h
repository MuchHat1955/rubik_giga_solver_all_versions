...
#define LOG_SECTION_START(title) \
  do { if (logging_on) log_section_start(title); } while (0)

#define LOG_SECTION_START_VAR(title, name, val) \
  do { if (logging_on) log_section_start_var(title, name, val); } while (0)

#define LOG_SECTION_END() \
  do { if (logging_on) log_section_end(); } while (0)
...
#define LOG_VAR(name, val) \
  do { if (logging_on) { log_indent(); Serial.printf("%s {%s}", name, String(val).c_str()); } } while (0)

#define LOG_VAR_CONT(name, val) \
  do { if (logging_on) { Serial.printf(" | %s {%s}", name, String(val).c_str()); } } while (0)

#define LOG_VAR2(n1, v1, n2, v2) \
  do { if (logging_on) { log_indent(); Serial.printf("%s {%s} | %s {%s}", n1, String(v1).c_str(), n2, String(v2).c_str()); } } while (0)

#define LOG_VAR2_CONT(n1, v1, n2, v2) \
  do { if (logging_on) { Serial.printf(" | %s {%s} | %s {%s}", n1, String(v1).c_str(), n2, String(v2).c_str()); } } while (0)
