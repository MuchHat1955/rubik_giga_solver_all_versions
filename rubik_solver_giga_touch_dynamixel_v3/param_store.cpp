...
#include <string>
#include <algorithm>
#include "flash_kvstore.h"

extern FlashKVStore prefs;
...
static void saveParamsToFlash() {
  for (auto& kv : store) {
    ...
    prefs.putInt(key, kv.second.value);
  }
}
...
static void loadParamsFromFlash() {
  ...
  for (auto& kv : store) {
    const char* key = kv.first.c_str();
    int saved = prefs.getInt(key, kv.second.value);  // return current if not saved
