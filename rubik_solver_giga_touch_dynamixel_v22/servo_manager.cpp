...
void ServoManager::updateAll(bool refreshHW) {
  LOG_SECTION_START("servo manager update all");

  // 1️⃣ Update each servo individually
  for (auto &kv : servosStore) {
    LOG_VAR("read servo", kv.first);
    updateServo(kv.first, refreshHW);
  }

  // 2️⃣ Update servo pose buttons (like "arm1_pose_0", etc.)
  for (int i = 0; poseKeys[i] != nullptr; ++i) {
    if (poseKeys[i][0] == '\0') continue;  // skip empty strings
    LOG_VAR("reflect ui (pose)", poseKeys[i]);
    reflectUI(poseKeys[i]);
  }

  // 3️⃣ Update group pose buttons (like "arms_home", "grippers_open", etc.)
    ...
    for (const auto &poseKey : poseList) {
      ...
      if (it != servosStore.end()) {
        ...
        if (issue) {
          ...
          break;  // stop early, one bad servo is enough
        }
      }
    }
...
void ServoManager::reflectUI(const String &key) {
  // TODO: update this to also work if key is a pose
  LOG_VAR("start reflect ui for key", key);

  // determine if key belongs to a pose, and extract servo base if so
  String justServoKey = key;
  String servoKeyPose = servoKeyFromPose(key.c_str());
  bool isServoPoseKey = false;

  if (servoKeyPose.length() > 0) {
    justServoKey = servoKeyPose;
    isServoPoseKey = true;
  }

  // lookup the servo info
  auto it = servosStore.find(justServoKey.c_str());
  if (it == servosStore.end()) {
    // LOG_VAR("key contains no servo nor a servo key", key);
    return;
  }

  // LOG_VAR("found servo from key", justServoKey);
  const ServoInfo &s = it->second;

  // evaluate servo state
  bool active = false;
  bool issue = (!s.pingOK) || s.isThermal || s.isStall || s.isAtMin || s.isAtMax;
  if (isServoPoseKey) {
    int servoPosGoal = getParamValue(key.c_str());
    active = abs(s.posTicks - servoPosGoal) <= POSE_TOL_TICKS;
  }

  LOG_VAR_("reflect ui for key", key);
  LOG_VAR_CONT_("is pose", isServoPoseKey);
  LOG_VAR_CONT_("has issue", issue);
  LOG_VAR_CONT("is active", active);

  // update UI button for this key
