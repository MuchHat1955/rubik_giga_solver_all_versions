...
void buttonAction_run(const char* btn_key) {
  ...
  if (!handled) handled = runDetectCubeByKey(btn_key, ok);
...
bool runDetectCubeByKey(const char* key, bool& result) {
  if (!key) return false;
  if (strcmp(key, "k_robot_moves_has_cube") == 0) return false;

  bool run_ok = runDetectCube();
  if (!run_ok) {
    LOG_ERR("[RUN] detect_cube_error=%s", getLastDetectCubeInfo().c_str());
    setFooter(("detect cube failed " + getLastDetectCubeInfo()).c_str(), _DONE_ERROR);
  } else {
    if (getLastDetectCubeResult()) setFooter("cube detected", _DONE_SUCCESS);
    else setFooter("cube not detected", _DONE_SUCCESS);
  }
  return true;
}
...
bool runRunOrientationByKey(const char* key, bool& result) {
  ...
  if (!result) {
    ...
    setFooter((cmd + " ok").c_str(), _DONE_SUCCESS);
  }
  return true;
}
...
bool runSolveCubeFindSolutionByKey(const char* key, bool& result) {
  ...
  if (colors_ok && solution_ok) {
    String footer_text = "solution found " + String(solution_move_count) + " moves";
