...
#include <string>
#include <algorithm>
#include <Arduino_NVM.h>

void saveInt(const char* key, int value) {
  NVM.setInt(key, value);
}

int loadInt(const char* key, int def = 0) {
  return NVM.getInt(key, 0);
}
...
static void loadParamsFromFlash() {
  prefs.begin("rubik", true);  // open namespace "rubik" for read

  for (auto& kv : store) {
    const char* key = kv.first.c_str();
    int saved = loadInt(key, kv.second.value);  // return current if not saved
    kv.second.value = saved;
  }
}
