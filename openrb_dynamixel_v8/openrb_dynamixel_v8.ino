...
void cmdMoveY(int rel_y_ticks) {
  ...
  cmdMoveAdaptive(ID_ARM2, goal2, goal1, 0);
  updateWristKeepBaseRef(arm2_start, wrist_start);
}
...
void cmdTestMoveY(float yStart_mm, float yEnd_mm, int steps = 10) {
  ...
  cmdMoveAdaptive(ID_ARM1, ticksStart, ticksStart, 0);
  cmdMoveAdaptive(ID_ARM2, ticksStart, ticksStart, 0);
  delay(300);
...
void loop() {
  ...
  if (U.startsWith("VERBOSEON")) {
    ...
    else if (verboseOn) Serial.println("Usage: INFO <id>");
  }
    ...
    if (sscanf(line.c_str(), "TESTMOVEY %f %f", &y1, &y2) == 2)
      cmdTestMoveY(y1, y2);
    else if (verboseOn) Serial.println("Usage: TESTMOVEY <yStart_mm> <yEnd_mm>");
  }
    ...
    else if (verboseOn) Serial.println("Usage: TESTMOVE <id> [cycles]");
  }

  else if (verboseOn) Serial.println("ERR: Unknown command");
