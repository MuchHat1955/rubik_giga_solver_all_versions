#pragma once
#include <Arduino.h>
#include <stdint.h>

#define LOG_ENABLE 1
#define DEBUG_ENABLE 0

// ===================================================================
// ENABLE / DISABLE LOGGING SYSTEMS
// ===================================================================
#ifndef LOG_ENABLE
#define LOG_ENABLE 1  // protocol / GIGA logs
#endif

#ifndef DEBUG_ENABLE
#define DEBUG_ENABLE 0  // internal verbose logs
#endif

// TODO MOD_COLORCHECK not used yet
// TODO   MOD_CUBEMOVE not used yet
// TODO    MOD_ROBOTMOVE not used yet

// ===================================================================
// MODULE IDS (shared)
// ===================================================================
enum log_module_t : uint8_t {
  MOD_CMD = 0,
  MOD_RUN,
  MOD_MOVE,
  MOD_SERVO,
  MOD_COLORSCAN,
  MOD_COLORCHECK,
  MOD_COLORSENSOR,
  MOD_CUBEORI,
  MOD_CUBEMOVE,
  MOD_ROBOTMOVE,
  MOD_KIN
};


// ===================================================================
// EXECUTION CONTEXT (implemented elsewhere)
// ===================================================================
void increment_cmd_no();

// ===================================================================
// LOW-LEVEL EMIT (implemented in log.cpp)
// ===================================================================
void log_begin(log_module_t e_module,
               int i_type,
               const char *description);

void log_kv(const char *key, int32_t v);
void log_kv(const char *key, int v);
void log_kv(const char *key, double v);
void log_kv(const char *key, const char *s);
void log_kv(const char *key, char c);
void log_kv(const char *key, const String &s);
void log_end();


// ===================================================================
// ======================= LOG_ (PROTOCOL) =============================
// ===================================================================

#define log_error_type 0
#define log_info_type 1
#define debug_error_type 2
#define debug_info_type 3

#if LOG_ENABLE

#define LOG_INFO(mod, tag) \
  do { log_begin(mod, log_info_type, tag); } while (0)

#define LOG_ERR(mod, tag) \
  do { log_begin(mod, log_error_type, tag); } while (0)

#define LOG_KV(key, val) \
  do { log_kv(key, val); } while (0)

#define LOG_END() \
  do { log_end(); } while (0)

#else

#define LOG_INFO(mod, tag) \
  do { \
  } while (0)
#define LOG_ERR(mod, tag) \
  do { \
  } while (0)
#define LOG_KV(key, val) \
  do { \
  } while (0)
#define LOG_END() \
  do { \
  } while (0)

#endif


// ===================================================================
// ======================= DEBUG_ (VERBOSE) ==============================
// ===================================================================
#if DEBUG_ENABLE

#define DEBUG_INFO(mod, tag) \
  do { log_begin(mod, debug_info_type, tag); } while (0)

#define DEBUG_ERR(mod, tag) \
  do { log_begin(mod, debug_error_type, tag); } while (0)

#define DEBUG_KV(key, val) \
  do { log_kv(key, val); } while (0)

#define DEBUG_END() \
  do { log_end(); } while (0)

#else

#define DEBUG_INFO(mod, tag) \
  do { \
  } while (0)
#define DEBUG_ERR(mod, tag) \
  do { \
  } while (0)
#define DEBUG_KV(key, val) \
  do { \
  } while (0)
#define DEBUG_END() \
  do { \
  } while (0)

#endif
