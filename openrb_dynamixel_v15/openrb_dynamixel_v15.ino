...
ServoConfig wrist("wrist", ID_GRIP, 2048, 1.0, 0, 4096);
...
public:
  ...
  double getGdeg() const {
    ...
    if (g_is_horiz) return 180 - a1_deg - a2_deg;
...
private:
  ...
  void update_from_angles() {
    ...
    // compute g
    double gr_rad = deg2rad(ticks2deg(ID_GRIP, dxl.getPresentPosition(ID_GRIP)));
    int g_abs_right_rad = a2abs_right_rad - gr_rad;
    // if closer to horizontal assume horizontal
    g_is_horiz = false;
    if (g_abs_right_rad < M_PI / 2) g_is_horiz = true;
  }
};
...
void print_status(uint8_t id) {
  ...
  if (dxl.ping(ID_ARM1) && dxl.ping(ID_ARM2)) {
    ...
    serial_printf("STATUS XY X=%.2fmm Y=%.2fmm A1=%.2fdeg A2=%.2fdeg G=%fdeg\n",  //
                  kin.getXmm(), kin.getYmm(), kin.getA1deg(), kin.getA2deg(), kin.getGdeg());
  } else {
...
bool cmdMoveYmm(double y_mm) {
  ...
  int currY = kin.getYmm();

  kin.setXYmm(0, y_mm);
  double goalA1_deg = kin.getA1deg();
  double goalA2_deg = kin.getA2deg();
  double goalG_deg = kin.getGdeg();

  // Verbose summary
  if (verboseOn) {
    serial_printf("MOVEY START goal=%.2f  currY=%.2f\n", y_mm, currY);
    serial_printf("   GOALS A1=%.2f  A2=%.2f  G=%.2f\n", goalA1_deg, goalA2_deg, goalA1_deg);
  }

  if (checkStallGroup({ ID_ARM1, ID_ARM2, ID_GRIP })) return false;

  // Perform vertical synchronized move
  cmdMoveYSyncSmooth(goalA1_deg, goalA2_deg, goalG_deg);
...
bool cmdMoveXmm(double x_mm) {
  ...
  int currG = dxl.getPresentPosition(ID_GRIP);
...
void setup() {
  ...
  Serial.println("---- OpenRB Arm Motion Controller v25 (Adaptive + Verbose + GripperComp) ----");
...
void loop() {
  ...
  // -------------- MOVE Y AXIS (absolute mm) --------------
    ...
    double val = 0;
    if (arduino_sscanf(line.c_str(), "%lf", &val) == 1) {
