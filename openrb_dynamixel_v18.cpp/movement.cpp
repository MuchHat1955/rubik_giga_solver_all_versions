...
// ensure below handles also the Gripper
bool cmdMoveSmoothXY(double goal_xmm, double goal_ymm, bool keepX, bool keepY) {
...
bool cmdMoveYmm(double y_mm) {
  ...
  // double g_goal_deg = kin.getGdeg_aligned(); // TODO this is going to be handled by XY function
  return cmdMoveSmoothXY(x_mm_curr, y_mm, true, false);
}
...
bool cmdMoveXmm(double x_mm) {
  ...
  return cmdMoveSmoothXY(x_mm, y_mm_curr, false, true);
}
