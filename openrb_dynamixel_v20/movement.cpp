  ...
  int getSyncGoal(uint8_t slaveIndex, int masterTicks) const {
    switch (mode) {
      ...
      case MODE_XY_HORIZONTAL: {
        ...
        // const_cast<VerticalKinematics*>(k)->solveFromA1(mode == MODE_XY_VERTICAL); // already done in setA1ticks
        if (slaveIndex == 1) return const_cast<VerticalKinematics*>(k)->getA2ticks();
        if (slaveIndex == 2) return const_cast<VerticalKinematics*>(k)->getGticks();
        return 0;
      }
