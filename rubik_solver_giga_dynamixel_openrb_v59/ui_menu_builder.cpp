  ...
  for (JsonArray row : rows) {
    ...
    for (int ci = 0; ci < numCols && ci < columns; ci++) {
      ...
      // ---------- SERVOS INFO ----------
        ...
        // --- special style for system text --------------------------------------------------------

        // Keep clickable so finger scrolling works
        lv_obj_add_flag(ta, LV_OBJ_FLAG_CLICKABLE);

        // CRITICAL: do NOT allow focus on click
        lv_obj_clear_flag(ta, LV_OBJ_FLAG_CLICK_FOCUSABLE);

        // Make absolutely sure it's not focused
        ...
        // Force cursor invisible (LVGL 8 style-based)
...
const char jsonBuffer[] = R"json(
{
  ...
  "k_robot_moves": {
    ...
    "rows": [
      [
        { "text": "cube:?", "type": "action", "key": "k_robot_moves_has_cube","status": "yes" },
        { "text": "f:?", "type": "action", "key": "k_robot_move_front_color","status": "yes" },
        { "text": "", "type": "text" }
      ],
      [
  ...
  "k_cube_moves": {
    ...
    "rows": [
      [
        { "text": "front color->", "type": "text", "key": ""  },
  ...
  "k_colors_and_orientation": {
    ...
    "rows": [
      [
        ...
        { "text": "ori na", "type": "text", "key": "k_orientation_val","status": "yes" },
        { "text": "", "type": "text", "key": "" }
      ],
      [
