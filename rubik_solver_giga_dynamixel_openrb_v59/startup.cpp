...
bool runDetectCubeStartupTests();
bool solver_begin();
...
static bool detect_cube_startup_tests_ok = false;
...
bool runStartupTests(bool force_rerun) {
  ...
  if (!force_rerun &&                                    //
      ...
      rb_begin_ok && solver_begin_ok && detect_cube_startup_tests_ok) {
    LOG_PRINTF_RUN("tests skipped | already run and ok\n");
  ...
  if (force_rerun) {
    ...
    detect_cube_startup_tests_ok = false;
    rb_begin_ok = false;
  ...
  if (!rb_begin_ok || !solver_begin_ok || force_rerun) {
    ...
    else begin_result += "rb err";
    begin_result += " | ";
    if (solver_begin_ok) begin_result += "solver ok";
    else begin_result += "solver err";
  ...
  // run more tests only if both begin are ok
  if (rb_begin_ok && solver_begin_ok) {
    ...
    if ((!detect_cube_startup_tests_ok || force_rerun) && rb_startup_tests_ok) {  // servos have to be ok
      setFooter("running... detect cube tests", _RUNNING_NOSTOP);
      detect_cube_startup_tests_ok = runDetectCubeStartupTests();
    }
    ...
    if (!detect_cube_startup_tests_ok) LOG_ERR("[RB] error=detect_cube_startup_tests_failed\n");

    String startup_tests_result = "tests | ";

    if (rb_startup_tests_ok) {
      startup_tests_result += "rb ok | ";
      if (detect_cube_startup_tests_ok) {
        if (getLastDetectCubeResult()) startup_tests_result += "has cube";
        else startup_tests_result += "no cube";
      } else startup_tests_result += "cube err";
    } else startup_tests_result += "rb err";

    startup_tests_result += " | ";
    if (solver_startup_tests_ok) startup_tests_result += "solver ok";
    else startup_tests_result += "solver err";

    if (!rb_startup_tests_ok || !solver_startup_tests_ok || !detect_cube_startup_tests_ok) setFooter(startup_tests_result.c_str(), _DONE_ERROR);
    else setFooter(startup_tests_result.c_str(), _DONE_SUCCESS);
    __delay(1333);
  }

  return rb_startup_tests_ok && solver_startup_tests_ok && detect_cube_startup_tests_ok;
}
...
bool runDetectCubeStartupTests() {
  LOG_PRINTF_RUN("start... detect cube tests\n");

  bool run_ok = runDetectCube();

  if (run_ok && getLastDetectCubeResult()) {
    LOG_PRINTF_RUN("cube detected\n");
    setFooter("cube detected", _DONE_SUCCESS);
  } else if (run_ok && !getLastDetectCubeResult()) {
    LOG_PRINTF_RUN("cube not detected {%s}\n", getLastDetectCubeInfo().c_str());
    String footer = "cube not detected " + getLastDetectCubeInfo();
    setFooter(footer.c_str(), _DONE_SUCCESS);
  } else {
    LOG_PRINTF_RUN("cube detection err\n");
    String footer = "cube detect error " + getLastDetectCubeInfo();
    setFooter(footer.c_str(), _DONE_ERROR);
  }
  LOG_PRINTF_RUN("end... detect cube tests\n");
  return run_ok;
}
