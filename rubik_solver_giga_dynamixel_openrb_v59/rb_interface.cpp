...
bool last_detect_cube_is_known = false;
...
static bool last_detect_cube_result = false;
static String last_detect_cube_error = "";
...
static void rb_info_cb(const String& mod,
  ...
  {
    String v;
    if (extract_kv(msg, "cube_detected=", v)) {
      if (v == "yes") last_detect_cube_result = true;
      else last_detect_cube_result = false;
      return;
    }
  }
  ...
  {
    String v;
    if (extract_kv(msg, "cube_detection_info=", v)) {
      last_detect_cube_error = v;
      return;
    }
  }
...
String getRbInterfaceVersion() {
  ...
  while (!cmd_states[id].finished_bool) {
    rb.poll();
    if (millis() - t0 > SERIAL_CMD_END_TIMEOUT) {
...
// bool means command was run
bool runDetectCube() {
  int cmd_id = -1;

  // look for detect_cube
  uint32_t id = rb.send_command("DETECTCUBE", "");
  cmd_id = (int)id;

  rb_cmd_state_t st;
  st.command_name = "DETECTCUBE";
  cmd_states[id] = st;
  ...
  while (!cmd_states[id].finished_bool) {
    rb.poll();
    if (millis() - t0 > SERIAL_CMD_END_TIMEOUT) {
      LOG_ERR("[RB] error=detectcube_command_timeout\n");
      LOG_ERR("[RB] id=%d finished=%d\n", id, cmd_states[id].finished_bool);
      last_detect_cube_is_known = false;
      return false;
    }
    delay(5);
  }
...
bool getLastDetectCubeResult() {
  return last_detect_cube_result;
}
bool getLastDetectCubeIsKnown() {
  return last_detect_cube_is_known;
}
String getLastDetectCubeInfo() {
  return last_detect_cube_error;
}
