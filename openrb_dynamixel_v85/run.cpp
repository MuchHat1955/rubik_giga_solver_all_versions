...
bool prepBaseForRotation(bool withCube) {
  ...
  RUN_CMD(cmdMoveXmm(X_ROTATE_BASE), "x rotate base");
  if (!isYmmAbove(Y_ROTATE_BASE)) {
    RUN_CMD(cmdMoveYmm(Y_ROTATE_BASE), "y rotate base");
  }
  RUN_CMD(cmdMoveXmm(X_ROTATE_BASE), "x rotate base");
  return true;
}
...
void timeout_stop_on() {
  ...
  if (millis() < millis_stop_set + 33000) return;

  LOG_INFO(MOD_RUN, "stop_set", "set_false_timeout");
