...
bool CubeColorReader::process_color_scan_step_(int step_index,
  ...
  LOG_INFO(MOD_COLORSCAN, "color_read_step", step_index);
  // Robot move (if any)
  if (robot_move && robot_move[0] != '\0' && strcmp(robot_move, "none") != 0) {
    // grip must be wide open
    if (!isGripperOpen(G_WIDE_OPEN))
      if (!cmdMoveGripperPer(G_WIDE_OPEN)) {
        LOG_ERR(MOD_COLORSCAN, "could not open the grip wide for step", step_index);
        return false;
      }
  ...
  LOG_INFO(MOD_COLORSCAN, "in_progress_cube_color_string_54", get_justread_color_string_54().c_str());  //NOW
...
String color_read_map_to_string(const color_map_step_t *orientation_map_ptr,
                                int total_steps) {
  if (orientation_map_ptr == nullptr) {
    LOG_ERR(MOD_COLORSCAN, "color read map", "null");
    return "err";
  }

  if (total_steps <= 0) {
    LOG_ERR(MOD_COLORSCAN, "color read map total steps", total_steps);
    return "err";
  }

  String out;
  out.reserve(total_steps * 4);  // small optimization

  for (int i = 0; i < total_steps; i++) {
    const color_map_step_t &step = orientation_map_ptr[i];

    // Skip steps that do not read colors (pure reposition)
    if (step.face == nullptr || step.face[0] == '\0') continue;
    if (step.order == nullptr || step.order[0] == '\0') continue;

    int count = strlen(step.order);
    if (count <= 0) continue;

    if (out.length() > 0) out += ' ';

    // face is a 1-char string ("f","r","u","d",...)
    out += step.face[0];
    out += count;
  }

  if (out.length() == 0) {
    LOG_ERR(MOD_COLORSCAN, "color read map", "no readable steps");
    return "err";
  }
...
bool CubeColorReader::read_cube(int scan_mode) {
  ...
  String color_read_map_str = color_read_map_to_string(orientation_map_ptr, total_steps);
  LOG_INFO(MOD_COLORSCAN, "color_read_start_with_map", color_read_map_str);
  LOG_INFO(MOD_COLORSCAN, "color_read_start_with_step_count", total_steps);
  LOG_INFO(MOD_COLORSCAN, "in_progress_cube_color_string_54", get_justread_color_string_54().c_str());  //should be empty for non-bottom
  ...
  for (int i = 0; i < total_steps; i++) {
    ...
    if (!process_color_scan_step_(i, s.robot_move, s.face, s.mirrored, s.order)) {
      ...
      if (scan_mode != SCAN_MODE_CENTERS) {
        ...
        clear_color_reader();
        fill_unknown_();
        LOG_INFO(MOD_COLORSCAN, "color_string_54", get_justread_color_string_54().c_str());  //should be empty for non-bottom
      }
      return false;
    }
  }
  // ~~~~~~~~~~~~~~~~ end scan ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  // data is only in the reader
  ...
  LOG_INFO(MOD_COLORSCAN, "ori_orientation", ori.get_orientation_string().c_str());
