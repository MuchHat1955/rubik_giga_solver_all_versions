...
bool CubeColorReader::bring_face_to_front_(char face) {
  ...
  if (!ori_.restore_orientation()) {
    serial_printf("ERR CubeColorReader: restore_orientation() failed\n");
    return false;
  }
...
bool CubeColorReader::read_full_cube() {
  ...
  for (int fi = 0; fi < 6; ++fi) {
    ...
    if (!bring_face_to_front_(f)) {
      ...
      ori_.restore_orientation();  // best effort to leave cube sane
      return false;
    }
  ...
  if (!ori_.restore_orientation()) {
    serial_printf("ERR CubeColorReader: final restore_orientation() failed\n");
    return false;
  }
